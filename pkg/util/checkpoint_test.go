...
func TestSigningRoundtripCheckpoint(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(string(test.c.Origin), func(t *testing.T) {
			...
			signerOpts := []signature.SignerVerifierOption{}
			...
			signer, _ := signature.LoadSignerWithOpts(test.signer, crypto.SHA256, signerOpts...)
			...
			if !test.wantSignErr {
				verifierOpts := []signature.SignerVerifierOption{}
				if rsaTestOpts, ok := test.opts.(*rsa.PSSOptions); ok && rsaTestOpts != nil {
					verifierOpts = append(verifierOpts, signature.WithRSAPSS(rsaTestOpts))
				}
				verifier, _ := signature.LoadVerifierWithOpts(test.pubKey, crypto.SHA256, verifierOpts...)
